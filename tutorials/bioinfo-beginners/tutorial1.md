...
Now go inside our `tutorials` directory by using: 
...
```bash
(base) letixa@eugenio tutorials % ls
(base) letixa@eugenio tutorials % 
```
...
* Type down a title for your README file (for example BIOINFORMATICS TUTORIALS). <br>
* Then use `Ctrl + X`, followed by Y and Enter to exit and save the file. <br>

Now, let's enter `ls` to see what's inside your `tutorials` folder. You should see a result like this:
```bash
(base) letixa@eugenio tutorials % ls
README.txt
```
...
```bash
(base) letixa@eugenio tutorials % cat README.txt 
...
**🐞 Moving a file:** \
Let's say you made a mistake and created the `README.txt` file inside `Documents`, instead of `tutorials`. Creating a file in the wrong folder is a very common mistake, so how can we correct it? \
From inside `Documents`, we can enter mv followed by the file name and the new location, like this:
```bash 
(base) letixa@eugenio Documents % mv README.txt tutorials
```
You can now navigate to `tutorials` and check if `README.txt` is there, like this:

```bash
(base) letixa@eugenio Documents % cd tutorials 
(base) letixa@eugenio tutorials % ls
README.txt           
````
Success!
...
(Before starting, make sure you are inside the `tutorials` folder)
